...
def add_rest_routes(routes, rag_instance):
    ...
    async def chat_endpoint(request: Request):
        try:
            ...
            mode = body.get("mode", "rag")  # "rag" | "general"

            if mode == "general":
                system = (
                    "Ты — умный универсальный ассистент. "
                    "Отвечай на любые вопросы, используя свои знания. "
                    "Если есть контекст из базы знаний — используй его как дополнительный источник. "
                    "Отвечай на том же языке, что и вопрос. Будь лаконичен и точен."
                )
                user_msg = (
                    f"Контекст из базы знаний (используй если релевантно):\n\n{context}\n\n---\n\nВопрос: {query}"
                ...
                system = (
                    ...
                    "Если контекст не содержит ответа — честно скажи об этом и предложи загрузить нужные документы. "
                    "Отвечай на том же языке, что и вопрос. Будь лаконичен и точен."
                )
                user_msg = (
